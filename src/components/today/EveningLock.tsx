...
export default function EveningLock({ user, topic, session, settings, onLock, onUnlock }: EveningLockProps) {
    ...
    return (
      <div className="space-y-4">
        <div className="bg-gray-900 border border-green-800 rounded-xl p-8 text-center">
          ...
          <h3 className="text-xl font-semibold text-white mb-2">Day {topic.day} Complete</h3>
        ...
        <div className="bg-gray-900 border border-gray-800 rounded-xl p-5 flex items-start justify-between gap-4">
          <div>
            <p className="text-sm font-medium text-gray-300">Need to reopen this day?</p>
          ...
          <button
            onClick={onUnlock}
            className="flex-shrink-0 bg-gray-800 hover:bg-gray-700 border border-gray-600 text-gray-300 hover:text-white text-sm font-medium px-4 py-2 rounded-lg transition-colors"
          >
            Reopen Day
          </button>
        </div>
  ...
  return (
    <div>
      <div className="mb-6">
        <h3 className="text-lg font-semibold text-white">Evening Lock</h3>
      ...
      <div className="bg-gray-900 border border-gray-800 rounded-xl p-5 mb-4">
        <h4 className="text-xs font-semibold text-gray-400 uppercase tracking-wider mb-4">Day Summary</h4>
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          ...
          <div className="text-center">
            <div className={`text-2xl font-bold ${session?.quiz_passed ? 'text-green-400' : 'text-red-400'}`}>
              {session?.quiz_passed ? 'Pass' : 'Fail'}
            </div>
            <div className="text-xs text-gray-500 mt-1">Quiz Result</div>
          </div>
          <div className="text-center">
            <div className="text-2xl font-bold text-white">{wrongQuestions.length}</div>
            <div className="text-xs text-gray-500 mt-1">Wrong Answers</div>
          </div>
          <div className="text-center">
            <div className={`text-2xl font-bold ${session?.mind_map_generated ? 'text-green-400' : 'text-gray-500'}`}>
              {session?.mind_map_generated ? 'Done' : 'Skip'}
            </div>
            <div className="text-xs text-gray-500 mt-1">Mind Map</div>
          </div>
        </div>
      </div>
      ...
      <div className="bg-gray-900 border border-gray-800 rounded-xl p-5 mb-6">
        ...
        {!cardsGenerated ? (
          <div>
            ...
            {error && (
              <div className="mb-3 bg-red-900/30 border border-red-800 rounded-lg px-3 py-2 text-red-400 text-sm">
                {error}
              </div>
            )}
            <button
              ...
              className="bg-purple-700 hover:bg-purple-600 disabled:bg-gray-700 disabled:cursor-not-allowed text-white font-medium px-5 py-2.5 rounded-lg transition-colors text-sm"
            >
              {loading === 'cards' ? (
      ...
      <div className="bg-gray-900 border border-gray-800 rounded-xl p-5">
        ...
        <button
          onClick={handleLock}
          disabled={loading !== null}
          className="bg-green-700 hover:bg-green-600 disabled:bg-gray-700 disabled:cursor-not-allowed text-white font-semibold px-6 py-3 rounded-lg transition-colors"
        >
          {loading === 'lock' ? (
            <span className="flex items-center gap-2">
              <span className="inline-block w-4 h-4 border-2 border-white border-t-transparent rounded-full animate-spin" />
